...
export class FormField extends Component {
  ...
  render() {
    const {title, value, placeholder, handleChangeText, otherStyles, keyboardType} = this.props

    return (
      <View className={`space-y-2 ${otherStyles}`}>
        <Text className="text-base text-gray-600 font-pmedium">{title}</Text>

        <View className="border-2 border-white w-full h-16 px-4 bg-white rounded-2xl focus:border-secondary items-center flex-row">
            <TextInput 
